...
Text: """
    ...
    def __post_init__(self):
        ...
        self.embed_batch_size = int(os.getenv("EMBED_BATCH_SIZE", self.embed_batch_size))
...
class NeuralClassifier:
    ...
    def _contrastive_loss(self, embeddings: torch.Tensor, labels: torch.Tensor, temperature: float = 0.1):
        """
    ...
    def _hard_negative_mining(self, X: np.ndarray, y: np.ndarray, n_neighbors: int = 5) -> np.ndarray:
        ...
        if hard_indices:
            logger.info(f"Found {len(hard_indices)} hard examples ({len(hard_indices)/len(y):.1%})")
    ...
    def validate(self, X: np.ndarray, y: np.ndarray, min_class_count: int) -> None:
        ...
        if len(X) >= 20 and min_class_count >= 2:
            ...
            skf = StratifiedKFold(n_splits=self.n_splits, shuffle=True, random_state=self.config.random_state)
            ...
            for train_idx, val_idx in skf.split(X, y):
                ...
                X_train_t = torch.FloatTensor(self._normalize(X_train, fit=True)).to(self.device)
                y_train_t = torch.LongTensor(y_train).to(self.device)
                X_val_t = torch.FloatTensor(self._normalize(X_val, fit=False)).to(self.device)
            ...
            logger.info(f"CV F1-macro: {self.cv_scores.mean():.3f} (±{self.cv_scores.std():.3f})")
        else:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        optimizer = torch.optim.AdamW(self.model.parameters(), lr=self.config.learning_rate, weight_decay=1e-5)
...
class Storage:
    ...
    def load_or_create_embeddings(self, embedding: Embedding) -> Tuple[np.ndarray, np.ndarray, int, int]:
        ...
        if cache_path.exists():
            ...
            labels = np.asarray([0] * len(human_texts) + [1] * len(ai_texts), dtype=np.int64)
...
class Evaluation:
    ...
    def evaluate(self, classifier, embedding, texts, true_labels):
        ...
        if misclassified:
            ...
            for ex in misclassified[:3]:  # Show first 3
                logger.info(f"  {ex['text'][:60]}... | True: {ex['true']} | Pred: {ex['pred']} (conf: {ex['confidence']:.2f})")
    ...
    def print_results(self, metrics: Dict[str, Any]) -> None:
        ...
        print(f"Class 1 (AI)    - P: {metrics['precision_ai']:.3f}, R: {metrics['recall_ai']:.3f}, F1: {metrics['f1_ai']:.3f}")
...
class TrustedText:
    ...
    def visualize(self, method: str = "umap", max_points: int = 500) -> None:
        ...
        if len(human_points) > 0 and len(ai_points) > 0:
            centroid_dist = np.linalg.norm(np.mean(human_points, axis=0) - np.mean(ai_points, axis=0))
    ...
    def save_stats(self, test_metrics: Optional[Dict[str, Any]] = None) -> Path:
        ...
        stats = {
            ...
            "cross_validation": {
                "scores": self.classifier.cv_scores.tolist() if self.classifier.cv_scores is not None else [],
                "mean_f1": float(self.classifier.cv_scores.mean()) if self.classifier.cv_scores is not None else 0.0,
                "std_f1": float(self.classifier.cv_scores.std()) if self.classifier.cv_scores is not None else 0.0,
            },
        }
        if test_metrics:
    ...
    def predict(self, text: str, return_prob: bool = True) -> Union[str, Tuple[str, float]]:
